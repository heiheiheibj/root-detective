 ...
 */
export type DictEntry = { word: string; phonetic: string; meaning: string; compound?: { parts: string[] } }
...
export async function lookupDict(word: string): Promise<DictResult> {
  ...
  try {
    ...
    const data = await res.json() as {
      ...
      phonetic?: string
      meaning?: string
      compound?: { parts: string[] }
      suggestions?: DictEntry[]
    }
    if (data.found) {
      return { entry: { word: data.word ?? q, phonetic: data.phonetic ?? '', meaning: data.meaning ?? '', compound: data.compound }, suggestions: [] }
    }
