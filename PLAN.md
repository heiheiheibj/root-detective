...
- 搜索收紧为「完整包含」：去掉子序列模糊匹配，只保留单词拼写/中文释义包含关键词；并新增**按词素含义搜索**（搜「看」出 vis/vid 等词），命中时「如何拆分」列的含义也会高亮。
- 词根表增强：每行加🔊发音按钮（用浏览器语音合成，受发音开关控制）；已学过的词打「✓ 已学」标签；通过 `completedWordIds` 与 `sound` 由 App 透传。
- 学完闭环：拼词学完后奖励页新增「回到词根表」按钮，返回时停在刚学的词根并高亮定位刚学的词（`lastStudiedWordId` + 滚动）。「今天」页的待复习块可点击直接进复习。
- 移动端：表格已在 ≤640px 转竖排卡片；侧栏在 ≤720px 变顶部横排导航；CaseRoom/世界地图已有 980/720/430 多档适配。连续打卡、成就、例句、例句发音此前已实现。
