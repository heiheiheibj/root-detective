...
- 词根表格加宽「如何拆分」（52%）与「单词意思」（20%）两列，「学习」列固定窄宽不换行，避免表头折行。
- 手机端布局：窄屏（≤640px）下表格转为竖排卡片——隐藏表头，每行一个卡片，每个单元格用 `data-label` 显示字段名（如何拆分 / 单词意思 / 学习），避免横向滚动把列挤成一团。清理了已废弃的卡片搜索样式。
- 搜索收紧为「完整包含」：去掉子序列模糊匹配（搜 age 不再命中 dialogue），只保留单词拼写包含关键词或中文释义包含关键词。
