# Stage 3 分批计划：300 词/60 词根 → **3,029 词 / 377 教学词素**

> 创建时间：2026-09-13（Stage 2 完成后）
> 最后更新：2026-09-13 —— **S4 定案：目标改用实测口径**（旧「250 词根 / 2000 词」已作废，见 §一之二）
> 前置阅读：`docs/Stage2-进度记录.md`、**`docs/测算方法说明.md`**（数据与口径）、**`docs/Stage3-执行TODO.md`**（执行进度）
>
> ## 目标（当前口径）
>
> | 维度 | 目标 | 说明 |
> |---|---|---|
> | 单词 | **3,029** | 所有零件都能拼出来的考试词（zk/gk/cet4/cet6 里可拆的部分） |
> | 教学词素 | **377** | ≥3 个考试词：有义项、有世界归属、可作词根卡 |
> | 零件词素 | **2,182** | 低频（<3 词）：只在拼词卡片里出现，不做教学单元 |
> | 词素合计 | **2,559** | = 377 教学 + 2,182 零件 |
>
> ## 批次划分（S4 定案，脚本 `scripts/tools/plan-batches.mjs`）
>
> 分批规则：词的层级取**最低** tag（zk > gk > cet4 > cet6）；词素归到**最早需要它的批次**。
>
...
>
> ⚠️ 每批 430~970 词，比 Stage 2（233 词/批）大 2~4 倍。实施时**按 ~150 词再分子批**（3.1a/3.1b/…），
> 子批边界在 S7 内容生产时按进度切。
>
> ⚠️ 教学词素在 3.1/3.2 就吃掉 347/377（92%）—— 因为常用词根先服务于简单词。
> 3.3/3.4 的增量主要是"零件词素"和少量长尾词根。
...
## 四、Stage 3.1 ~ 3.4：按考试层级分批加词

**批次表见文档开头**（3.1 中考 434 词 → 3.2 高考 974 → 3.3 四级 771 → 3.4 六级 850）。
每批做完就有一层实际可用性；每批实施时按 ~150 词再分子批（见 §五 流水线）。

> 旧方案（250 词根 / 2000 词 / 5 批）已作废 —— 它基于「406 条候选词根表」估算，
> S1 用 Wiktionary 全量重建词素库后，实测口径完全不同（详见 §一之二 与 `docs/测算方法说明.md`）。
