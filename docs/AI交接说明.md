...
> ⚠ **本文档第 2、3 节写的是 Stage 0 / Stage 1 的状态，都已成历史。**
> **当前状态看 [`docs/Stage3-进度交接.md`](Stage3-进度交接.md)** —— Stage 2 / Stage 3 也已完成，
> 现在词库是 2698 词 / 2144 词素 / 42 世界。第 4~7 节的坑与许可红线仍然全部有效，一定要读。
...
## 3.（历史）Stage 1 已完成（67 词 / 20 词根，管线全绿）
...
> Stage 2 / Stage 3 后来都做完了，这一节保留作历史记录。当前进度见
> [`docs/Stage3-进度交接.md`](Stage3-进度交接.md)。
...
下面这段是**复核用**的，不是接手干活用的 —— 场景是「换一个 AI 独立看一遍刚做完的一轮改动」。
接手继续开发的版本见 8.2。
...
词库已经扩到 2698 词 / 2144 词素 / 42 世界，Stage 0~3 全部完成。

我请你**独立复核最近 4 个提交**（3adf204 → 8e721f7），不是接着干活。
请先自己跑出基线，再判断，**不要接受我复述的任何数字**：

  npm test && npm run validate:content

先读这些，再动手：
- docs/Stage3-进度交接.md   ← 当前状态 + 本轮改动的完整记录（重点读「词素重复记录」一节）
- docs/AI交接说明.md        ← 第 4~7 节的坑与许可红线，仍然全部有效
- scripts/tools/build-stage3-config.mjs   ← 本轮的改动集中在这个文件的收敛点
- scripts/lib/morpheme-fallback.mjs       ← OVERRIDE_MEANINGS / DICT_ARTIFACT_RE

本轮做了什么（背景，不是结论）：
词素表里同一个词根存了两条记录（21 号按 cigen 的拼法定 id，与切分算法定的 id 不同拼法），
导致词详情页同一词根两个名字、地图上同一词根出两张卡。排查中发现这些记录的义项有一半是
「词典把词素当英文缩写/专名查了」的垃圾（trah=人名特拉汉、dc=医直电流、minim=量滴液量单位）。
分三层修：0) 85 处义项强制覆盖；1) 合并 8 个残渣 id；2) 合并 3 组跨世界同根。

请重点攻击这几处（我自己知道最可能出问题的地方）：
1. **85 处覆盖义项是我一个人拍的，没有任何第三方校验过。** 逐个核对
   `morpheme-fallback.mjs` 的 `OVERRIDE_MEANINGS` 值与它的家族词是否真的相符。
   有疑问的列出来。
2. 「not 家族不合并、而是把 notice/notation 改挂 note」是个编辑判断。
   现在 not(不) 的家族是 neither/notwithstanding，note(知道、标记) 的家族是
   notice/notation/notebook/notable —— 判断这个划分对不对。
3. 合并后家族变大（cor 11 词、mit 10 词、jud 8 词），
   确认没有词因此挂到了语义不符的词根下。
4. 核对波及面：与 `3adf204^` 逐词对比 `words.json` 的 parts，
   应当**恰好只有 20 个词变化**、词数 2698 不变。多一个少一个都要报。
5. 新增的 A20b / A24b 只报警不算错，正则覆盖面有限 —— 评估这样够不够，
   还是应该升成 error。

输出要求：分「确认无误 / 存疑待定 / 建议修改」三类，每条给出文件行号或命令输出作为依据。
不确定就说不确定，不要替我圆场。

硬约束：
- 用户不读英文、不做人工校对，所以别设计「请用户看一眼这句英文」的环节。
- 跟用户汇报用中文。
- 别动 PROFILE_VERSION。别在 src/domain/contentRules.ts 里写带值的相对 import（详见 4.1）。
- `npm run content:all` 是零成本、可反复跑的；但它会重写 src/domain/content/ 下的所有产物，
  复核时以只读为主，要跑先确认 git status 干净。
```
...
先读这三个文件，再动手：
- docs/Stage3-进度交接.md   ← 当前在哪、还剩什么（Stage 0~3 全完成，2698 词 / 2144 词素 / 42 世界）
- docs/AI交接说明.md        ← 第 4~7 节的坑与许可红线（必读，第 2~3 节是历史）
...
2. 跟用户汇报用中文。
3. Stage 0~3 都已完成（53 测试通过；2698 词管线全绿、可全量重跑、$0 花销）。
   交接文档末尾列了剩余可做的事，按价值排序。
